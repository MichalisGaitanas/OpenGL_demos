...
int main()
{
    ...
    std::vector<float> brightness_vector_cpu;
    std::vector<float> brightness_vector_gpu;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        brightness_vector_cpu.push_back(get_brightness_cpu(tex_lightcurve, win_width, win_height));
        brightness_vector_gpu.push_back(get_brightness_gpu(tex_lightcurve, win_width, win_height));
        t += dt; //[sec]
        if (t >= 200.0f)
        {
            break;
            time_vector.erase(time_vector.begin());
            brightness_vector_cpu.erase(brightness_vector_cpu.begin());
            brightness_vector_gpu.erase(brightness_vector_gpu.begin());
        }
        ...
        if (ImPlot::BeginPlot("Lightcurve", plot_win_size))
        {
            ...
            ImPlot::PlotLine("", time_vector.data(), brightness_vector_cpu.data(), time_vector.size());
    ...
    for (size_t i = 0; i < brightness_vector_cpu.size(); ++i)
        fprintf(fp,"%f  %f  %f\n",time_vector[i], brightness_vector_cpu[i], brightness_vector_gpu[i]);
    fclose(fp);
