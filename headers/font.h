...
public:
    ...
    font(const char *path)
    {
        ...
        {
            cout << "Error while initializing FreeType library. Exiting...\n";
            exit(EXIT_FAILURE);
        }
        //load font as face
        FT_Face face;
        ...
        for (GLubyte c = 0; c < 128; c++)
        {
            ...
            {
                cout << "Failed to load current glyph. Proceeding to the next\n";
                continue;
            }
            //generate texture
    ...
    void draw(string text, float x, float y, int winWidth, int winHeight, float scale, glm::vec3 color, shader &shad)
    {
        ...
        shad.set_vec3_uniform("textCol", color);
        ...
        for (c = text.begin(); c != text.end(); c++)
        {
            ...
            float ypos = y - (ch.glyphSize.y - ch.bearing.y)*scale;
            ...
            glBindTexture(GL_TEXTURE_2D, ch.texID); //render glyph texture over quad
