...
camera cam(glm::vec3(0.0f, -37.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f), 90.0f);
...
int main()
{
    ...
    meshvfn gerasimenko("../obj/vfn/pyramid.obj");
    meshvfn_ebo gerasimenk("../obj/vfn/pyramid_smooth.obj");
    //meshvfn gerasimenko("../obj/vfn/asteroids/gerasimenko256k_smooth.obj");
    //meshvfn_ebo gerasimenko("../obj/vfn/asteroids/gerasimenko256k_smooth.obj");
    printf("meshvfn: interleaved_buffer size = %zu\n", gerasimenko.interleaved_buffer.size());
    printf("meshvfn_ebo: interleaved_buffer size = %zu, index count = %zu\n", gerasimenk.interleaved_buffer.size(), gerasimenk.inds.size());
    getchar();
    ...
    glm::vec3 light_pos = glm::vec3(0.0f,-10.0f,30.0f); //Light position in world coordinates.
    ...
    while (!glfwWindowShouldClose(window)) //Game loop.
    {
        ...
        projection = glm::perspective(glm::radians(cam.fov), (float)win_width/win_height, 0.11f,700.0f);
        cam.move(time_tick);
        view = cam.view();
        sponza_shad.use();
        ...
        for (int i = -15; i < 15; i+=4)
        {
            ...
            {
                model = glm::mat4(1.0f);
                model = glm::translate(model, glm::vec3((float)i, 5.0f,(float)j));
                model = glm::rotate(model, (float)glfwGetTime(), glm::vec3((float)i,(float)j,0.0f));
