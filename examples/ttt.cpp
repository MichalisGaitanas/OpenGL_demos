...
float aspectRatio;
bool fullscreen = true; //not at fullscreen initially
...
int main()
{
    ...
    if (fullscreen)
    {
        ...
        {
            if (modes[i].width > winWidth)
            {
                winWidth = modes[i].width;
                winHeight = modes[i].height;
            }
        }
    }
    GLFWwindow* const window  = glfwCreateWindow(winWidth, winHeight, "AngryBots OpenGL", monitor, NULL);
