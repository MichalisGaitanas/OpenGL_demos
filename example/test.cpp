...
void key_callback(GLFWwindow *win, int key, int scancode, int action, int mods)
{
    if (key == GLFW_KEY_M && action == GLFW_PRESS)
        mount_dimorphos = !mount_dimorphos;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(win))
    {
        ...
        {
            //find a way to compute the camera's position and aim
            view = glm::lookAt(glm::vec3((M1/(M1 + M2))*state[0],(M1/(M1 + M2))*state[1],(M1/(M1 + M2))*state[2] + 0.075f),
